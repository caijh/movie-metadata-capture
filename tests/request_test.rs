#[cfg(test)]
mod tests {
    use movie_metadata_capture::{
        config::AppConfig,
        request::{client, get_html_content},
    };
    use std::error::Error;
    use url::Url;
    ...
    async fn test_get_html_content() {
        AppConfig::load_config_file("./Config.toml")
            .await
            .expect("Fail to load Config.toml, Please check config file.");
        ...
        let detail_url = format!(
            "{}{}",
            "https://www.dmm.co.jp/mono/dvd/-/detail/=/cid=", "ka9oae232"
        );
        ...
        let html = if result.is_ok() { result.ok() } else { None };
        println!("{:?}", html);
        assert!(html.is_some());
    }
